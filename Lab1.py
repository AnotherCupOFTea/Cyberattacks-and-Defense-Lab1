...
print("Loaded:", 'http.cap', "Packets:", packet)
...
if len(packet) > 3:
    ...
    print("The capture contains fewer than 4 packets \n")
    
if Raw in p:
    print(bytes(p[Raw].load))
else:
...
if TCP in p:
    ...
    modified.show()
    modified[TCP].dport = 8035
    modified.show()
else:
    print("The selected packet has no TCP layer")
...
# Yes, the port has indeed changed to 35
...
# The TCP dport has changed to "domain"

# Task 8

p = p = IP(dst="8.8.8.8") / UDP(dport=53) / DNS()
p.show()
